...
for epoch in range(params.num_epochs):
    ...
    # random num of train tgt classfier

    train_cls_tgt = random.random() <= p_tgt
    # same for target domain
    if train_cls_tgt:
        print("use  label_tgt to trian classfier")
        cnt_use_tgt_lable+=1
        optimizer_c.zero_grad()
    ...
    if ((epoch + 1) % params.save_step == 0):
        save_model(critic, "V4_WGAN-GP_critic-{}.pt".format(epoch + 1))
        save_model(classifier,
                   "V4_WGAN-GP_classifier-{}.pt".format(epoch + 1))
        save_model(generator, "V4_WGAN-GP_generator-{}.pt".format(epoch + 1))
        save_model(generator_larger,"V4_WGAN-GP_generator_larger-{}.pt".format(epoch + 1))

print("cnt_use_tgt_label", cnt_use_tgt_lable)
